...
export async function createAnalysisReport(input: AnalysisReportInput): Promise<AnalysisReportResult> {
  const reportDir = input.outDir ?? path.join(input.runDir, 'report');
  const indexPath = path.join(reportDir, 'index.html');
  const data = await readReportData(input.runDir);
  await mkdir(reportDir, { recursive: true });
  await writeFile(indexPath, renderReportHtml(input.runDir, reportDir, data));
  return { reportDir, indexPath };
}

async function readReportData(runDir: string): Promise<ReportData> {
...
function renderReportHtml(runDir: string, reportDir: string, data: ReportData): string {
...
function renderTransitions(transitions: unknown[]): string {
  ...
  return table(['Time', 'From → To', 'Type', 'Evidence'], transitions.map((transition) => {
    const item = asRecord(transition);
    return [
      `${stringValue(item.timestampSec, '0')}s`,
      `${stringValue(item.fromSceneIndex, '?')} → ${stringValue(item.toSceneIndex, '?')}`,
...
function renderCaptions(captions: unknown[]): string {
  ...
  return table(['Frame', 'Text', 'Confidence'], captions.slice(0, 12).map((caption) => {
    const item = asRecord(caption);
    return [escapeText(stringValue(item.frameName, '')), escapeText(stringValue(item.text, '')), escapeText(stringValue(item.confidence, ''))];
  }));
}
